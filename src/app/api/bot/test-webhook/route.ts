...
// Initialize bot with inline handlers
...
bot.command('start', async (ctx) => {
  try {
    ...
    const welcomeMessage = `👋 *Welcome ${escapeMarkdown(username)}\\!*
...
bot.command('list', async (ctx) => {
  try {
    ...
    products.forEach((product: any, index: number) => {
      ...
      message += `${index + 1}\\. [${escapeMarkdown(product.name.substring(0, 40))}](${escapeMarkdown(product.url)})\n`;
      message += `   💰 Current: £${escapeMarkdown(price.toFixed(2))} \\| 🎯 Alert: £${escapeMarkdown(threshold.toFixed(2))}\n`;
...
bot.on('text', async (ctx) => {
  ...
  if (text.match(/amazon\.|amzn\./i)) {
    ...
    await ctx.reply('Send me an Amazon product URL to track\\!', { parse_mode: 'MarkdownV2' });
  } else if (text === '❓ Help') {
...
export async function GET() {
  return NextResponse.json({ 
    message: 'Bot Webhook Endpoint (Full Version)',
    status: 'active'
  });
}
